...
def write_pcs_to_config(settings: rset.Settings, config: cfg.RandoConfig):
    # Choose the character at each recruit spot.
    #   * LEGACY_OF_CYRUS uses a forced assignment (story-locked).
    #   * CHAR_RANDO shuffles characters across the recruit spots.
    #   * Otherwise we use the base recruit dict's vanilla assignments
    #     as-is (see characters/pcrecruit.get_base_recruit_dict).
    # Previously the shuffle ran unconditionally, which meant "no flags"
    # still produced randomized character placements -- surprising to
    # users who toggled char-rando off expecting vanilla behavior.
    ...
    elif rset.GameFlags.CHAR_RANDO in settings.gameflags:
    ...
    # Layer 2: stats/model/palette reassignment. This used to run for
    # any --char-rando seed, but it's the source of "wrong palette"
    # surprises (Crono's slot playing as Magus's stats/model). The
    # apworld interpretation is now: layer 1 (above) handles "shuffle
    # which character is at which recruit spot," and layer 2 only
    # kicks in when the player explicitly opts into duplicate
    # characters (which inherently requires reassigning slot
    # identities). DUPLICATE_CHARS forces CHAR_RANDO on via
    # ForcedFlags so this branch implicitly requires both flags.
