...
def add_check_to_ozzies_fort_script(ct_rom: ctrom.CTRom):
    ...
    get_item_func = (
        ...
        .add(EC.auto_text_box(new_ind))
        # Archipelago/Jets-of-Time-local tracking: reserve bit 0x02 of the
        # Ozzie's Fort state byte (0x7F01A1) for the KI pickup event so the
        # SNIClient can auto-detect it. Existing bits on this byte:
        #   0x01 = met Ozzie, 0x04 = Flea+ defeated, 0x08 = Super Slash defeated
        # 0x02 was unused; the matching apworld reads it from Client.py.
        .add(EC.set_bit(0x7F01A1, 0x02))
    )
