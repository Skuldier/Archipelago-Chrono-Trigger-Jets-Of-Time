...
class CTJoTLocationManager:
    ...
    def add_filler_locations(self, regions_from_config, game_mode, player: int, region: Region):
        ...
        # Experimental-flag KI spots (Bekkler Key, Cyrus Grave Key, Arris Dome
        # Food Locker Key, Ozzies Fort Key, Sun Keep 2300, Lab 32 Race Log,
        # rocksanity rocks) are added to the YAML when the matching flag is
        # enabled, but are not part of every game mode's chronosanity pool.
        # Tolerate their absence from location_ids: they have already been
        # created as KI spots in create_regions() and don't need filler slots.
        for location_entry in config_locations:
            loc_id = self._name_to_id_mapping[location_entry] - self._LOCATION_ID_START
            if loc_id in location_ids:
                location_ids.remove(loc_id)
